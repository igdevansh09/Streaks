...
export default function PushScreen() {
  ...
  const { token, tokenError, registering, register, permissionStatus } = usePushNotifications();
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.bg, paddingTop: insets.top }}>
      ...
      <View style={{ paddingHorizontal: 20, paddingTop: 16, paddingBottom: 12 }}>
        <Text style={{ fontSize: 11, fontWeight: '600', letterSpacing: 1, color: c.text3, textTransform: 'uppercase', marginBottom: 4 }}>
          SERVER PUSH
        </Text>
        <Text style={{ fontSize: 32, fontWeight: '700', color: c.text }}>Push</Text>
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <View style={{ paddingHorizontal: 20, paddingTop: 20, paddingBottom: 8 }}>
          <Text style={{ fontSize: 11, fontWeight: '600', letterSpacing: 0.8, textTransform: 'uppercase', color: c.text3, marginBottom: 12 }}>
            Status
          </Text>
          <View style={{
            flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
            backgroundColor: c.bg2, borderRadius: 10, padding: 14, borderWidth: 0.5, borderColor: c.border,
          }}>
            <Text style={{ fontSize: 14, fontWeight: '500', color: c.text }}>Notification permission</Text>
            <View style={{
              paddingHorizontal: 10, paddingVertical: 3, borderRadius: 20,
              backgroundColor: permissionStatus === 'granted' ? c.success : c.danger,
            }}>
        ...
        <View style={{ paddingHorizontal: 20, paddingTop: 12, paddingBottom: 8 }}>
          ...
          <Text style={{ fontSize: 12, color: c.text3, lineHeight: 18, marginBottom: 12 }}>
            Push notifications do NOT work in Expo Go. Build with EAS ({' '}
            <Text style={{ fontFamily: Platform.OS === 'ios' ? 'Courier' : 'monospace', color: c.text2 }}>
          ...
          <TouchableOpacity
            style={{
              padding: 14, borderRadius: 8, backgroundColor: c.text, alignItems: 'center',
          ...
          >
            {registering
              ...
              : <>
                  ...
                  <Text style={{ fontSize: 13, fontWeight: '700', color: c.bg, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                    Register for Push
                  </Text>
                </>
            }
            ...
            <View style={{ marginTop: 12 }}>
              ...
              <TouchableOpacity
                style={{
                  ...
                  justifyContent: 'center', gap: 6, borderWidth: 0.5, borderColor: c.border2,
                }}
                onPress={copyToken}
              >
                <Ionicons name="copy-outline" size={16} color={c.text} />
                <Text style={{ fontSize: 13, fontWeight: '700', color: c.text, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                  Copy Token
                </Text>
              </TouchableOpacity>
            </View>
          ...
          {tokenError && (
            <View style={{
              ...
              backgroundColor: c.bg2, borderWidth: 0.5, borderColor: c.danger,
            }}>
              <Text style={{ fontSize: 12, color: c.danger, lineHeight: 18 }}>{tokenError}</Text>
            </View>
          )}
        </View>
        ...
        <View style={{ marginHorizontal: 20, marginTop: 8, marginBottom: 4 }}>
          ...
          <View style={{
            backgroundColor: c.bg2, borderRadius: 10, padding: 14,
            ...
            <Text style={{ fontSize: 12, color: c.text3, lineHeight: 18, marginBottom: 12 }}>
              Paste your token into the Expo push tool and send with this JSON body:
            </Text>
            <View style={{
              backgroundColor: c.bg3, borderRadius: 6, padding: 10,
              borderWidth: 0.5, borderColor: c.border,
            }}>
              <Text style={{
            ...
            <Text style={{ fontSize: 13, fontWeight: '600', color: c.text, marginTop: 14, marginBottom: 6 }}>
              Option B — cURL
            </Text>
            <View style={{
              backgroundColor: c.bg3, borderRadius: 6, padding: 10,
            ...
            <Text style={{ fontSize: 12, color: c.text3, lineHeight: 18 }}>
              ...
              {' '}from the repo root. Handles tickets, receipts, and DeviceNotRegistered drops automatically.
            </Text>
          </View>
        </View>

        {/* ── Foreground vs Background explanation ── */}
          ...
          <View style={{ gap: 8 }}>
            {[
              ...
              <View key={item.label} style={{
                backgroundColor: c.bg2, borderRadius: 10, padding: 14,
                borderWidth: 0.5, borderColor: c.border, flexDirection: 'row', gap: 12,
              }}>
                <Ionicons name={item.icon} size={20} color={item.color} style={{ marginTop: 2 }} />
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 13, fontWeight: '600', color: c.text, marginBottom: 4 }}>
                    {item.label}
                  </Text>
                  <Text style={{ fontSize: 12, color: c.text3, lineHeight: 18 }}>{item.desc}</Text>
                </View>
              </View>
            ))}
          </View>
        ...
        <View style={{ marginHorizontal: 20, marginTop: 16 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <Text style={{ fontSize: 11, fontWeight: '600', letterSpacing: 0.8, textTransform: 'uppercase', color: c.text3 }}>
              Received pushes ({received.length})
            </Text>
            {received.length > 0 && (
              <TouchableOpacity onPress={() => setReceived([])}>
                <Text style={{ fontSize: 12, color: c.danger, fontWeight: '600' }}>Clear</Text>
              </TouchableOpacity>
            )}
          </View>
          ...
          {received.length === 0 ? (
            <View style={{
              padding: 20, borderRadius: 10, backgroundColor: c.bg2,
              ...
              <Text style={{ fontSize: 12, color: c.text3, textAlign: 'center', lineHeight: 18 }}>
                Send a test push using the instructions above. Received pushes appear here with their app state.
              </Text>
            </View>
          ) : (
            received.map((entry) => (
              ...
              >
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                  ...
                  <View style={{
                    ...
                    backgroundColor: entry.appState === 'foreground' ? c.success : c.text3,
                  }}>
                    <Text style={{ fontSize: 10, fontWeight: '700', color: '#fff' }}>
                      {entry.appState === 'foreground' ? 'FOREGROUND' : 'BG/KILLED'}
                    </Text>
                  </View>
                </View>
                <Text style={{ fontSize: 12, color: c.text3, marginBottom: 4 }}>{entry.body}</Text>
                <Text style={{ fontSize: 11, color: c.text3 }}>Received at {entry.receivedAt}</Text>
                {entry.habitId && (
                  <Text style={{
                    ...
                    {entry.habitId !== 'test' ? '  · tap to open →' : '  · (test id, no habit)'}
                  </Text>
                )}
              </TouchableOpacity>
            ))
